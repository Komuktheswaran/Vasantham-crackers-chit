...
const Payments = () => {
  ...
  const generateReferenceId = async () => {
    try {
      ...
      console.warn("Backend sequence API failed, calculating sequence locally via getAll():");
      try {
         const allPayRes = await paymentsAPI.getAll();
         const allPayments = allPayRes.data.data?.payments || allPayRes.data?.payments || allPayRes.data.data || allPayRes.data || [];
         ...
         console.error("Both primary and fallback sequence generation failed", fallbackErr);
  ...
  return (
    <div className="page-container">
      ...
      <Modal
        ...
        style={{ top: 20 }}
        styles={{
          body: { maxHeight: "75vh", overflowY: "auto", paddingTop: 8 },
        }}
      >
        <Form form={form} layout="vertical" onFinish={onFinish} size="small" style={{ rowGap: 0 }}>
          <Form.Item
            name="dueNumber"
            label="Due Number"
            rules={[{ required: true }]}
          ...
          <Form.Item name="transactionId" label="Payment Reference ID" style={{ marginBottom: 10 }}>
            <Input
              readOnly
              placeholder="Autogenerated Reference ID (YYYY/NNNNN)"
            />
          </Form.Item>
